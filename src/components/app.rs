...
struct IcedWorkspacesApplet {
    ...
    inactive_pill_style_model: segmented_button::SingleSelectModel,
...
fn inactive_pill_style_model(style: WorkspacePillStyle) -> segmented_button::SingleSelectModel {
    let mut model: segmented_button::SingleSelectModel = segmented_button::Model::builder()
        .insert(|button| {
            button
                .text(crate::fl!("pill-style-filled"))
...
impl IcedWorkspacesApplet {
    ...
    fn sync_inactive_pill_style_model(&mut self) {
        self.inactive_pill_style_model
            .activate_position(match self.config.inactive_pill_style {
                WorkspacePillStyle::Filled => 0,
                WorkspacePillStyle::Outlined => 1,
            });
    }
    ...
    fn workspace_pill_style(
        ...
        urgent: bool,
        hovered: bool,
        pill_style: WorkspacePillStyle,
        inactive_pill_style: WorkspacePillStyle,
        outlined_border_width: f32,
        inactive_contrast_percent: u8,
    ) -> container::Style {
        let cosmic = theme.cosmic();
        let urgent = urgent && !active;
        let (background, text_color, border_color, border_width) = if active && pill_style == WorkspacePillStyle::Outlined {
            ...
            (
                (pill_style != WorkspacePillStyle::Outlined || hovered).then_some(Background::Color(color)),
                destructive,
                destructive,
                if pill_style == WorkspacePillStyle::Outlined {
                    outlined_border_width
                } else {
            ...
            (
                (inactive_pill_style == WorkspacePillStyle::Filled || hovered).then_some(Background::Color(background)),
                component.on.into(),
                if inactive_pill_style == WorkspacePillStyle::Outlined {
                    background
                } else {
                    Color::TRANSPARENT
                },
        ...
        let border_color = if pill_style == WorkspacePillStyle::Outlined && hovered && !urgent {
        ...
        let border_width = if pill_style == WorkspacePillStyle::Outlined && hovered && !urgent {
            0.0
        } else {
            border_width
        };
    ...
    fn workspace_number_style(
        ...
        if !active || pill_style != WorkspacePillStyle::Outlined {
    ...
    fn workspace_divider_style(
        ...
        let color = if active && pill_style == WorkspacePillStyle::Outlined {
    ...
    fn workspace_pill_visual<'a>(
        ...
        let pill_style = self.config.pill_style;
        let inactive_pill_style = self.config.inactive_pill_style;
        ...
        let number = container(number_text).class(ContainerClass::Custom(Box::new(
            move |theme| Self::workspace_number_style(theme, active, pill_style, hovered),
        )));
...
mod tests {
    ...
    fn test_pill_style(
        ...
        urgent: bool,
        hovered: bool,
        active_pill_style: WorkspacePillStyle,
        inactive_pill_style: WorkspacePillStyle,
    ...
    fn applies_configured_contrast_to_filled_and_outlined_inactive_pills() {
        ...
        for outlined_mode in [WorkspacePillStyle::Filled, WorkspacePillStyle::Outlined] {
            for (configured, resting_contrast, hovered_contrast) in
                [(55, 55, 70), (90, 90, 100), (0, 0, 15)]
            {
                let resting = test_pill_style_with_contrast(
                    &theme,
                    false,
                    WorkspacePillStyle::Filled,
                    outlined_mode,
                ...
                let hovered = test_pill_style_with_contrast(
                    &theme,
                    true,
                    WorkspacePillStyle::Filled,
                    outlined_mode,
                ...
                if outlined_mode == WorkspacePillStyle::Outlined {
    ...
    fn keeps_active_and_urgent_styles_independent_of_inactive_contrast() {
        ...
        for (active, urgent, hovered, outlined_mode) in [
            (true, false, false, WorkspacePillStyle::Filled),
            (true, false, true, WorkspacePillStyle::Outlined),
            (false, true, false, WorkspacePillStyle::Filled),
            (false, true, true, WorkspacePillStyle::Outlined),
        ] {
    ...
    fn gives_the_active_number_an_accent_text_color_in_outlined_mode() {
        let theme = Theme::default();
        let style = IcedWorkspacesApplet::workspace_number_style(&theme, true, WorkspacePillStyle::Outlined, false);
        let expected = theme
            .cosmic()
            .accent_text
    ...
    fn gives_the_active_number_on_accent_text_when_hovered() {
        let theme = Theme::default();
        let style = IcedWorkspacesApplet::workspace_number_style(&theme, true, WorkspacePillStyle::Outlined, true);
    ...
    fn gives_the_active_divider_an_accent_color_in_outlined_mode() {
        let theme = Theme::default();
        let style =
            IcedWorkspacesApplet::workspace_divider_style(&theme, true, false, WorkspacePillStyle::Outlined, false);
        let expected = theme
            .cosmic()
            .accent_text
            .unwrap_or(theme.cosmic().accent_button.base);
    ...
    fn gives_the_active_divider_an_on_accent_color_when_hovered() {
        let theme = Theme::default();
        let style =
            IcedWorkspacesApplet::workspace_divider_style(&theme, true, false, WorkspacePillStyle::Outlined, true);
...
enum Message {
    ...
    InactivePillStyle(segmented_button::Entity),
    PillBorderWidth(u8),
    PillSpacing(u8),
...
impl cosmic::Application for IcedWorkspacesApplet {
    ...
    fn init(core: cosmic::app::Core, _flags: Self::Flags) -> (Self, app::Task<Self::Message>) {
        ...
        let inactive_pill_style_model = inactive_pill_style_model(config.inactive_pill_style);
    ...
    fn update(&mut self, message: Self::Message) -> app::Task<Self::Message> {
        match message {
            ...
            Message::InactivePillStyle(entity) => {
                if let Some(style) = self
                    .inactive_pill_style_model
                    .data::<WorkspacePillStyle>(entity)
                    .copied()
                {
                    self.inactive_pill_style_model.activate(entity);
                    self.config.inactive_pill_style = style;
                    self.write_config();
                }
            }
            ...
            Message::ConfigUpdated(mut config) => {
                ...
                self.sync_inactive_pill_style_model();
            }
            Message::Surface(a) => {
    ...
    fn view_window(&self, _id: window::Id) -> Element<'_, Message> {
        ...
        let outline_thickness: Element<'_, Message> =
            if self.config.pill_style == WorkspacePillStyle::Outlined || self.config.inactive_pill_style == WorkspacePillStyle::Outlined {
                row![
                    self.core
                        .applet
        ...
        let content = column![
            ...
            padded_control(
                column![
                    ...
                    column![
                        self.core.applet.text(crate::fl!("inactive-pill-style")).size(14),
                        segmented_control::horizontal(&self.inactive_pill_style_model)
                            .width(Length::Fill)
                            .on_activate(Message::InactivePillStyle),
                        outline_thickness
                    ]
                    .spacing(spacing.space_xxs)
                    .align_x(Alignment::Start),
